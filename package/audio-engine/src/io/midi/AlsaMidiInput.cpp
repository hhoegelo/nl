...
AlsaMidiInput::AlsaMidiInput(const std::string &deviceName, Callback cb)
    : MidiInput(cb), m_cancellable(Gio::Cancellable::create()) {
  open(deviceName);
}

AlsaMidiInput::~AlsaMidiInput() { close(); }

void AlsaMidiInput::open(const std::string &deviceName) {
  if (snd_rawmidi_open(&m_handle, nullptr, deviceName.c_str(),
...
void AlsaMidiInput::start() {
  m_run = true;
  m_task =
      std::make_unique<HighPriorityTask>(2, [this]() { doBackgroundWork(); });
}
...
void AlsaMidiInput::doBackgroundWork() {
  ...
  numPollFDs =
      snd_rawmidi_poll_descriptors(m_handle, pollFileDescriptors, numPollFDs);
  ...
  while (true) {
    ...
    case POLLNVAL:
      throw std::runtime_error(
          "Polling the midi input file descriptor failed. Terminating.");
    ...
    while (m_run) {
      ...
      if (readResult == 1) {
        ...
        nltools::throwException(
            "Could not read from midi input file descriptor =>",
            snd_strerror(readResult));
      }
    }
  }
}
