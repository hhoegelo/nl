...
struct Param_Handle {
  ...
  constexpr static uint32_t m_layer_count =
      static_cast<uint32_t>(Layer::_LENGTH_);
  ...
  inline void init_modMatrix() {
    ...
    // manually setting up constant return behaviors for bender (4), aftertouch
    // (5)
    m_global.m_source[4].m_behavior =
  ...
  inline Macro_Param *init_macro_time(const C15::ParameterDescriptor _element) {
    auto param = get_macro(_element.m_param.m_index);
    param->m_time.init(_element.m_ae.m_scaleId, _element.m_ae.m_scaleFactor,
                       _element.m_ae.m_scaleOffset, _element.m_initial);
    param->m_time.m_initial = _element.m_initial;
    return param;
  }
  inline void
  init_global_modulateable(const C15::ParameterDescriptor _element) {
  ...
  inline void
  init_global_unmodulateable(const C15::ParameterDescriptor _element) {
  ...
  inline void
  init_local_unmodulateable(const C15::ParameterDescriptor _element) {
    for (uint32_t i = 0; i < m_layer_count; i++) {
  ...
  inline Direct_Param *get_local_direct(const uint32_t _layerId,
                                        const uint32_t _id) {
    return &m_layer[_layerId].m_direct[_id];
  }
  inline Target_Param *get_local_target(const uint32_t _layerId,
                                        const uint32_t _id) {
  ...
  inline Direct_Param *get_local_unison_voices(const Layer _layerId) {
    return &m_layer[static_cast<uint32_t>(_layerId)]
                .m_direct[static_cast<uint32_t>(
                    C15::Parameters::Local_Unmodulateables::Unison_Voices)];
  }
  inline Direct_Param *get_local_mono_enable(const Layer _layerId) {
    return &m_layer[static_cast<uint32_t>(_layerId)]
                .m_direct[static_cast<uint32_t>(
                    C15::Parameters::Local_Unmodulateables::Mono_Grp_Enable)];
  }
  inline Direct_Param *get_local_mono_priority(const Layer _layerId) {
    return &m_layer[static_cast<uint32_t>(_layerId)]
                .m_direct[static_cast<uint32_t>(
                    C15::Parameters::Local_Unmodulateables::Mono_Grp_Prio)];
  }
  inline Direct_Param *get_local_mono_legato(const Layer _layerId) {
    return &m_layer[static_cast<uint32_t>(_layerId)]
                .m_direct[static_cast<uint32_t>(
                    C15::Parameters::Local_Unmodulateables::Mono_Grp_Legato)];
  }
  ...
  inline Target_Param *localChainFirst(const uint32_t _layer,
                                       const uint32_t _mcId) {
