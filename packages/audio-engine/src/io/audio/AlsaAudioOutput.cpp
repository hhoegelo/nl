...
AlsaAudioOutput::AlsaAudioOutput(const AudioEngineOptions *options,
                                 const std::string &deviceName, Callback cb)
    : m_cb(cb), m_options(options) {
  open(deviceName);
}

AlsaAudioOutput::~AlsaAudioOutput() { close(); }
...
void AlsaAudioOutput::open(const std::string &deviceName) {
  unsigned int sampleRate =
      static_cast<unsigned int>(m_options->getSampleRate());
  ...
  checkAlsa(snd_pcm_hw_params_set_access(m_handle, hwparams,
                                         SND_PCM_ACCESS_RW_INTERLEAVED));

  if (checkAlsa(snd_pcm_hw_params_set_format(m_handle, hwparams,
  ...
  checkAlsa(snd_pcm_hw_params_set_rate_near(m_handle, hwparams, &sampleRate,
                                            nullptr));
  ...
  snd_pcm_uframes_t framesPerPeriod =
      static_cast<snd_pcm_uframes_t>(m_options->getFramesPerPeriod());
  checkAlsa(snd_pcm_hw_params_set_period_size_near(m_handle, hwparams,
                                                   &framesPerPeriod, nullptr));

  unsigned int channels = 0;
  checkAlsa(snd_pcm_hw_params_get_channels(hwparams, &channels));

  snd_pcm_uframes_t ringBufferSize =
      static_cast<snd_pcm_uframes_t>(m_options->getAlsaRingBufferSize());
  checkAlsa(snd_pcm_hw_params_set_buffer_size_near(m_handle, hwparams,
                                                   &ringBufferSize));
...
void AlsaAudioOutput::start() {
  ...
  m_run = true;
  m_task =
      std::make_unique<HighPriorityTask>(0, [this]() { doBackgroundWork(); });
}
...
void AlsaAudioOutput::doBackgroundWork() {
  ...
  auto microsPerBuffer =
      std::micro::den * framesPerCallback / m_options->getSampleRate();
